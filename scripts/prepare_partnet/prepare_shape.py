...
def get_shape_info(index, lev, root):

    fn = os.path.join(root, str(index), "result_after_merging.json")
    with open(fn, "r") as f:
        json_infos = json.load(f)[0]
    parts_objs, parts_names = get_parts_objs(json_infos, lev, '')
    
    parts_v = []
    parts_f = []
    for part_objs in parts_objs:
        obj_fns = [os.path.join(root, str(index), "objs", obj + ".obj") for obj in part_objs]
        vs = []
        fs = []
...
if __name__ == "__main__":
    root_to_load_data = "/data00/home/zhangrufeng1/datasets/partnet/Chair_6323"
    root_to_load_file = "/data00/home/zhangrufeng1/projects/3DPartAssembly/data/partnet_dataset"
    root_to_save_file = "/data00/home/zhangrufeng1/datasets/partnet/zhang_partnet/shape_data"
    cat_name = "Chair"
    modes = ["train", "val", "test"]
    levels = [3, 1, 2]
    

    fn_hier = os.path.join(root_to_load_file, "stats/after_merging_label_ids", cat_name + "-hier.txt")
    ...
    for level in levels:
        ...
        fn_level = os.path.join(root_to_load_file, "stats/after_merging_label_ids", cat_name + "-level-" + str(level) + ".txt")
        ...
        for mode in modes:
            ...
            mode_file = cat_name + "." + mode + ".json"
            object_json = json.load(open(os.path.join(root_to_load_file, "stats/train_val_test_split", mode_file)))
            object_list = [int(object_json[i]['anno_id']) for i in range(len(object_json))]


            for i, fn in enumerate(object_list):
                print("Convert {}/{}: level-{}-mode-{}-id-{}".format(i, len(object_list), level, mode, fn))

                parts_pcs, Rs, ts, parts_names, sizes = get_shape_info(fn, lev, root_to_load_data)
                ...
                dic_to_save = {"part_pcs": parts_pcs, "part_poses": parts_poses,
                               "part_ids": parts_ids, "geo_part_ids": geo_part_ids, "sym": sym}
                if not os.path.exists(root_to_save_file):
                    os.makedirs(root_to_save_file)
                path_to_save_file = os.path.join(root_to_save_file, str(fn) + "_level" + str(level) + ".npy")
                np.save(path_to_save_file, dic_to_save)
