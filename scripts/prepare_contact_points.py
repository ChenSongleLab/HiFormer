...
def check_data_consistance(newdata, shape_id):
    olddir = "/media/chensl/data/shanglei/prepare_data/"
    cur_contact_data_fn = os.path.join(olddir, 'contact_points/pairs_with_contact_points_%s_level'
                                           % shape_id + "3" + '.npy')
    cur_contacts = np.load(cur_contact_data_fn, allow_pickle=True)
    if (newdata-cur_contacts).any() !=0:
        print("error")
    else:
        print("check okay")
...
if __name__ == "__main__":
    root = "/media/chensl/data/shanglei/prepare_data/shape_data"
    root_to_save = "/media/chensl/data/chensl/assemble_sw_gpu2_fixbackbone/3d_assembly/data/partnet_dataset/"
    cat_name = "Chair"
    modes = ['val','train','test']
    split_path = "/media/chensl/data/chensl/assemble_sw_gpu2_fixbackbone/3d_assembly/data/partnet_dataset/stats/"
    levels = [3]
    for level in levels:
        for mode in modes:
            object_json =json.load(open(split_path + "train_val_test_split/" + cat_name +"." + mode + ".json"))
            ...
            for id in object_list:
                ...
                if True:
                    ...
                    check_data_consistance(newfile, id)
