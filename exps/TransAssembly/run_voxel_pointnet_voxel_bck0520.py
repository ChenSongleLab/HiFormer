...
parser.add_argument('--data_dir', type=str, default='/media/user/data1/shanglei/prepare_data', help='data directory')
...
parser.add_argument('-j', '--workers', default=15, type=int, metavar='N', help='number of data loading workers (default: 32)')
parser.add_argument('--start-epoch', default=0, type=int, metavar='N', help='manual epoch number (useful on restarts)')
parser.add_argument('--epochs', default=330, type=int, metavar='N', help='number of total epochs to run')
...
parser.add_argument('--lr_backbone', '--backbone-learning-rate', default=5e-4, type=float,
                    metavar='LRBackbone', help='backbone initial (base) learning rate')
parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                    metavar='W', help='weight decay (default: 1e-6)',
                    dest='weight_decay')

parser.add_argument('--output-dir', default="/media/user/data1/chensl/assemble_run/backbone20240513/voxelres250max/", type=str)
parser.add_argument('-p', '--print-freq', default=20, type=int,
                    metavar='N', help='print frequency (default: 10)')
parser.add_argument('--save-freq', default=1, type=int,
                    metavar='N', help='save frequency (default: 10)')
parser.add_argument('--resume', default="/media/user/data1/chensl/assemble_run/backbone20240513/voxelres250max/checkpoint.pth.tar", type=str, metavar='PATH',
...
parser.add_argument('--group_tm_pred_with_higher_box', type=int, default=1)
...
parser.add_argument('--backbone', default='minkowski', type=str, help='pointnet_cls, minkowski, swin_transformer, point_voxel, the backbone to extract init features.')
...
parser.add_argument('--train-mon', default=1, type=int, help='MoN iterations in training.')
parser.add_argument('--eval-mon', default=1, type=int, help='MoN iterations in inference.')
parser.add_argument('--noise-cat', default=0, type=int, help='Whether to cat noise when prediction.')
parser.add_argument('--noise-cat-in-encoder', default=0, type=int, help='Whether to cat noise when in trans-encoder.')
...
parser.add_argument('--num_heads', default= [2, 4, 8, 16], type=list, help='')
...
parser.add_argument('--classes', default=20, type=int, help='这个是和每个点对应的特征类似的，得到每个点的类别')
...
parser.add_argument("--inplanes", default=32, type=int)
...
def main():
    ...
    args.lr_backbone_names = ["pre_encoder"]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        batch_finished, batch_finished_list = train_one_epoch(train_loader, iter_loader, model, optimizer, scaler, summary_writer,
                                    log_writer, epoch, countdown, args, batch_finished_list, batch_finished)
        torch.cuda.empty_cache()
        show_process_gpu_info("call train_one_epoch return, finised batches:%d" %(batch_finished))

        while batch_finished < len(train_loader) - 1:
            batch_finished, batch_finished_list  = train_one_epoch(train_loader, iter_loader, model, optimizer, scaler, summary_writer,
                                             log_writer, epoch, countdown, args, batch_finished_list, batch_finished+1)
            torch.cuda.empty_cache()
            show_process_gpu_info("call train_one_epoch return, finised batches:%d" % (batch_finished))
        ...
        if (epoch + 1) % args.save_freq == 0 or epoch + 1 == args.epochs:
            if (epoch + 1) >= args.eval_epochs:
                part_acc = eval_func(val_loader, model, log_writer, summary_writer, epoch, args)
            else:
                part_acc = 0.
...
def testOCR():
    ...
    filename = '/mnt/sda/dp/python-ocr10.jpg'
...
def testOCR2():
    ...
    import easyocr
    for i in range(11):
        filename = '/mnt/sda/dp/python-ocr' + str(i+1)+'.jpg'
