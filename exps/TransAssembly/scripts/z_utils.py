...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    if epoch < args.warmup_epochs:
        ...
        lr_backbone = lr_backbone * epoch / args.warmup_epochs
    else:
        lr = lr * 0.5 * (1. + math.cos(math.pi * (epoch - args.warmup_epochs) / (args.epochs - args.warmup_epochs)))
        lr_backbone = lr_backbone * 0.5 * (1. + math.cos(math.pi * (epoch - args.warmup_epochs) / (args.epochs - args.warmup_epochs)))
    if epoch > args.warmup_epochs and lr < 0.00001:
        lr = 0.00001
    if lr_backbone !=0 and epoch > args.warmup_epochs and lr_backbone < 0.00001:
        lr_backbone = 0.00001
    optimizer.param_groups[0]['lr'] = lr

    return lr, lr_backbone
...
def save_model(epoch, scaler, output_dir, model, optimizer, part_acc, best_acc, args):
    if (epoch + 1) % args.save_freq == 0 or epoch + 1 == args.epochs:
        if args.rank == 0:
            ...
            }, is_best = False, filename = osp.join(output_dir, 'checkpoint.pth.tar'))
