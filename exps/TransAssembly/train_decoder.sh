...
ENCODE_FREEZE=1
TYPE_EVAL=decoder
RESUME=/pretrained/chair_encoder6_ep1000_noise64/checkpoint_best.pth.tar
NUM_POS=1
NUM_QUERIES=1
COEF_DECODE=0.1
MEMORY_DETACH=1
...
INS_CAT_IN_DECODER=1
POSE_CAT_IN_MEMORY=0
NOISE_CAT_IN_MEMORY=0


OUTPUT_DIR=${CURDIR}/checkpoints/chair_decoder6_ep500_noise64_baseline


python3 -m torch.distributed.launch --master_port=$((RANDOM + 10000)) --use_env --nproc_per_node=${NUM_GUPS} ${CURDIR}/run.py --category ${CATEGORY} \
...
--model ${MODEL} --enc_layers ${ENC_LAYERS} --dec_layers ${DEC_LAYERS} \
...
--ins-cat-in-decoder ${INS_CAT_IN_DECODER} --pose-cat-in-memory ${POSE_CAT_IN_MEMORY} --noise-cat-in-memory ${NOISE_CAT_IN_MEMORY} \
--encode-freeze ${ENCODE_FREEZE} --type-eval ${TYPE_EVAL} --resume ${RESUME}
