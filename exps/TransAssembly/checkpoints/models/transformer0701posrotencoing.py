...
class TEncoder(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerEncoderStandardLayer(d_model, nhead, dim_feedforward,
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src,
        ...
        for idx, (parts_layer, in_group_layer, out_group_layer) in enumerate(zip(self.parts_layers, self.in_group_layers, self.out_group_layers)):
            ...
            emd_rot_x = torch.gather(self.rot_3d_x, dim=0, index=rot_xyz_quant_index[:, 0].long().unsqueeze(-1).repeat(1, dim))
            emd_rot_y = torch.gather(self.rot_3d_y, dim=0, index=rot_xyz_quant_index[:, 1].long().unsqueeze(-1).repeat(1, dim))
            emd_rot_z = torch.gather(self.rot_3d_z, dim=0, index=rot_xyz_quant_index[:, 2].long().unsqueeze(-1).repeat(1, dim))
            emd_rot = emd_rot_x + emd_rot_y + emd_rot_z
            output = output + emd_rot.view(num_part, batch_size, dim)


            output = parts_layer(output, src_mask=mask, src_key_padding_mask=src_key_padding_mask, pos=pos)
