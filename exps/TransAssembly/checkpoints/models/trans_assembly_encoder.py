...
class TransAssembly_encoder(nn.Module):
    ...
    def forward(self, shape_id, part_pcs, part_boxes, part_valid, gt_part_poses, match_ids, part_ids, num_group, num_group_part, parts_valid_in_group, \
        ...
        if self.args.backbone == "pointnet_cls":
        ...
        elif self.args.backbone == "minkowski":
            part_pcs_1 = part_pcs.view(batch_size * num_part, -1, 3)
            point_clouds = [part_pcs_1[i] for i in range(part_pcs_1.shape[0])]
            base_feat = self.feature_extract_minkowski(point_clouds)
            trans_loss = None
