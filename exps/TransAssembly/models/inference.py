...
def inference(preds, gt_poses, part_pcs, part_valids, match_ids, contact_points, sym_info, args=None):
    ...
    pred_poses_per_trans_contact = pred_poses_per_trans.clone()
    ...
    for bs_ind in range(batch_size):
        ...
        for ins_id in range(1, num_part + 1):
            ...
            matched_pred_ids_contact = matched_pred_ids.copy()
            parts_num = len(matched_pred_ids_contact)
            for gt_index in range(parts_num):
                for j in range(parts_num):
                    if matched_gt_ids[j] == gt_index:
                        pre_index = matched_pred_ids[j]
                        matched_pred_ids_contact[gt_index] = pre_index
                        break
            pred_poses_per_trans_contact[bs_ind, need_to_match_part] = cur_pred_poses[matched_pred_ids_contact]
    ...
    pred_trans_contact = pred_poses_per_trans_contact[:, :, :3]
    pred_rot_contact  = pred_poses_per_trans_contact[:, :, 3:]




    contact_point_loss_per_trans, num_correct_contact, num_contact_points = get_contact_point_loss(pred_trans_contact, pred_rot_contact,
                                                                                                   contact_points,
                                                                                                   sym_info)
    acc = acc.sum(-1).float()
