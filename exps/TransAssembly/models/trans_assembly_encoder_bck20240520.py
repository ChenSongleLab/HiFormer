...
class TransAssembly_encoder(nn.Module):
    ...
    def feature_extract_swin_transformer(self, part_pcs, parts_valid):
        ...
        output = self.swin_tm(feat, coord, offset, batch, neighbor_idx)
        ...
        time_start = time.time()
        return out_feat, dense2flat_index
    ...
    def forward(self, shape_id, part_pcs, part_boxes, part_valid, gt_part_poses, match_ids, part_ids, num_group, num_group_part, parts_valid_in_group, \
        ...
        if "minkowski" in self.args.backbone:
            feat_zero = torch.zeros([num_part, 256, 1], dtype=torch.float32, device=part_pcs.device, requires_grad=True)
            base_feat = []
            for i in range(batch_size):
                part_pcs_list = [part_pcs[i][j] + 0.5 for j in range(int(part_valid[i].sum()))]
                pm = len(part_pcs_list)
                feat_part = self.feature_extract_minkowski(part_pcs_list)
                feat_zero_fill = feat_zero[0:num_part - int(part_valid[i].sum())]
                feat = torch.cat([feat_part, feat_zero_fill], dim=0).unsqueeze(0)
                base_feat.append(feat)
            base_feat_voxel = torch.cat(base_feat)
            base_feat_voxel = base_feat_voxel.view(batch_size, num_part, -1)
            base_feat_list.append(base_feat_voxel)
            trans_loss = None
        if  "swin_transformer" in self.args.backbone:
            base_feat_swing , dense2flat_index = self.feature_extract_swin_transformer(part_pcs, part_valid)
    ...
    def feature_extract_minkowski(self, point_clouds):
        ...
        for batch_id in range(batch_num):
            ...
            features_pool = self.maxpool(features_batch_squ)
