...
from .models import MLP, Predictor, MLPRelu, PredictorBox, AttnRelativePosition


import os
import sys
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../utils'))
from quaternion import qeuler, qrot, qmul
from .box_ops import *
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
        ...
        encoder_layer = TransformerRelPosEncoderLayer(d_model, nhead, dim_feedforward,
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, encoder_layer, encoder_layer_std, num_layers, norm=None, return_intermediate=False, args=None):
        ...
        if args.box_vertex_cat_in_encoder:
            dim_in += 8*3

        mlp_encoder = MLPRelu(dim_in, args.feat_dim*4, args.feat_dim, 3)
        self.proj_encoder_in = nn.ModuleList([mlp_encoder for _ in range(num_layers)])
        ...
        if not args.shared_pred:
            ...
            if args.cascade_pred:
                self.predictor_init = _get_clones(predictor, 1)
                self.enc_output = nn.Linear(args.feat_dim, args.feat_dim)
                self.enc_output_norm = nn.LayerNorm(args.feat_dim)
        else:
            self.mlp = mlp
        ...
        self.window_size = 2
        self.quant_size = 0.1
        self.quant_grid_length = int(self.window_size / self.quant_size)
        self.pose_3d_x = nn.Parameter(torch.zeros(2 * self.quant_grid_length - 1, d_model))
        self.pose_3d_y = nn.Parameter(torch.zeros(2 * self.quant_grid_length - 1, d_model))
        self.pose_3d_z = nn.Parameter(torch.zeros(2 * self.quant_grid_length - 1, d_model))

        self.rot_window_size = 3.2
        self.rot_quant_size = 0.1
        self.rot_quant_grid_length = int(self.rot_window_size / self.rot_quant_size)
        self.rot_3d_x = nn.Parameter(torch.zeros(2 * self.rot_quant_grid_length - 1, d_model))
        self.rot_3d_y = nn.Parameter(torch.zeros(2 * self.rot_quant_grid_length - 1, d_model))
        self.rot_3d_z = nn.Parameter(torch.zeros(2 * self.rot_quant_grid_length - 1, d_model))
        self._reset_parameters()
    ...
    def embedding_pred_center_rotate(self, output, pred, num_part, batch_size, dim):
        ...
        xyz = pred[:, :, 0:3]
        xyz = torch.clamp(xyz, -self.window_size, self.window_size)
        xyz_quant = xyz // self.quant_size
        xyz_quant_index = xyz_quant + self.quant_grid_length - 1
        xyz_quant_index = xyz_quant_index.contiguous().view(num_part * batch_size, 3)
        emd_pos_x = torch.gather(self.pose_3d_x, dim=0, index=xyz_quant_index[:, 0].long().unsqueeze(-1).repeat(1, dim))
        emd_pos_y = torch.gather(self.pose_3d_y, dim=0, index=xyz_quant_index[:, 1].long().unsqueeze(-1).repeat(1, dim))
        emd_pos_z = torch.gather(self.pose_3d_z, dim=0, index=xyz_quant_index[:, 2].long().unsqueeze(-1).repeat(1, dim))
        emd_pos = emd_pos_x + emd_pos_y + emd_pos_z
        output = output + emd_pos.view(num_part, batch_size, dim)

        rot_xyz = pred[:, :, 3:]
        rot_xyz = qeuler(rot_xyz, "xyz")
        rot_xyz = torch.clamp(rot_xyz, -3.14, 3.14)
        rot_xyz_quant = rot_xyz // self.rot_quant_size
        rot_xyz_quant_index = rot_xyz_quant + self.rot_quant_grid_length - 1
        rot_xyz_quant_index = rot_xyz_quant_index.contiguous().view(num_part * batch_size, 3)
        emd_rot_x = torch.gather(self.rot_3d_x, dim=0,
                                 index=rot_xyz_quant_index[:, 0].long().unsqueeze(-1).repeat(1, dim))
        emd_rot_y = torch.gather(self.rot_3d_y, dim=0,
                                 index=rot_xyz_quant_index[:, 1].long().unsqueeze(-1).repeat(1, dim))
        emd_rot_z = torch.gather(self.rot_3d_z, dim=0,
                                 index=rot_xyz_quant_index[:, 2].long().unsqueeze(-1).repeat(1, dim))
        emd_rot = emd_rot_x + emd_rot_y + emd_rot_z
        output = output + emd_rot.view(num_part, batch_size, dim)

        return output


    def relative_box_position_encoding(self, box):
        batch_size, num_part, _ = box.size()
        box = box.reshape(batch_size, num_part, 8, 3)

        box_v = box.unsqueeze(3).repeat(1, 1, 1, 8, 1)
        box_v_v = box_v.unsqueeze(2).repeat(1, 1, num_part, 1, 1, 1)
        box_h = box.unsqueeze(2).repeat(1, 1, 8, 1, 1)
        box_h_h = box_v.unsqueeze(1).repeat(1, num_part, 1, 1, 1, 1)
        box_rel = box_v_v - box_h_h
        box_rel = box_rel.view(batch_size, num_part, num_part, -1)
    ...
    def forward(self, src,
        ...
        box_vertex_input = box_xyxy_to_vextex(box_cxcywh_to_xyxy(part_boxes))
        ...
        box_vertex_input = box_vertex_input.permute(1, 0, 2, 3)
        if self.args.cascade_pred:
            src_init  = self.enc_output_norm(self.enc_output(src))
            _, cum_trans, cum_quat = self.predictor_init[0](src_init)
            cum_pred = torch.cat([cum_trans, cum_quat], dim=-1)
            intermediate_part_pose.append(cum_pred.permute(1, 0, 2))
            intermediate_part_box.append( src.new_zeros(batch_size, num_part, 6) )
            intermediate_group_box.append(src.new_zeros(batch_size, num_group, 6))
            intermediate_shape_box.append(src.new_zeros(batch_size,  6))
            if self.args.pred_detach:
                with torch.no_grad():
                    cum_pred = cum_pred.detach()
                    cum_trans = cum_trans.detach()
                    cum_quat = cum_quat.detach()

            center2 = cum_pred[:, :, 0:3].unsqueeze(2).repeat(1, 1, 8, 1)
            quat2 = cum_pred[:, :, 3:]
            box_vertex2 = qrot(quat2.unsqueeze(2).repeat(1, 1, 8, 1),
                               box_vertex_input.view(num_part, batch_size, 8, 3)) + center2
            box_vertex = box_vertex2 * parts_valid.permute(1, 0).unsqueeze(2).unsqueeze(3).repeat(1, 1, 8, 3)
        else:
            cum_trans = src.new_zeros(num_part, batch_size, 3)
            cum_quat = src.new_zeros(num_part, batch_size, 4)
            cum_quat[:, :, 0] = 1
            cum_pred = torch.cat([cum_trans, cum_quat], dim=-1)
            box_vertex = box_vertex_input
        ...
        for idx, (parts_layer, in_group_layer, out_group_layer) in enumerate(zip(self.parts_layers, self.in_group_layers, self.out_group_layers)):
            ...
            if self.args.pose_cat_in_encoder:
                output = torch.cat((output, cum_pred), dim=-1)
            ...
            if self.args.box_vertex_cat_in_encoder:
                box_vertex = box_vertex.view(num_part, batch_size, -1)
                output = torch.cat((output, box_vertex), dim=-1)
            output = self.proj_encoder_in[idx](output)


            if self.args.embedding_pred_center_rotate:
                output = self.embedding_pred_center_rotate(output, cum_pred, num_part, batch_size, dim)


            box_rel = None
            if self.args.relative_box_position_encoding:
                box_rel = self.relative_box_position_encoding(box_vertex.permute(1,0,2))

            output = parts_layer(output, box_rel, src_mask=mask, src_key_padding_mask=src_key_padding_mask, pos=pos)
            ...
            if self.args.pose_cat:
                feat = torch.cat((feat, cum_pred), dim=-1)
            ...
            if not self.args.shared_pred:
                ...
                pred, pred_trans, pred_quat = self.predictor[idx](feat)
                pred_box = self.mlp_box_part[idx](feat)
            else:
                feat = self.mlp(feat)
                pred_tmp = self.predictor(feat)

            if self.args.cascade_pred:
                cum_trans = qrot(pred_quat, cum_trans) + pred_trans
                cum_quat = qmul(pred_quat, cum_quat)
                cum_pred = torch.cat([cum_trans, cum_quat], dim=-1)
            else:
                cum_pred = pred
            ...
            if self.args.pred_detach:
                with torch.no_grad():
                    ...
                    pred_quat = pred_quat.detach()
            if self.args.cascade_pred:

                center1 = pred_trans
                center1 = center1.unsqueeze(2).repeat(1, 1, 8, 1)
                quat1 = pred_quat
                box_vertex1 = qrot(quat1.unsqueeze(2).repeat(1, 1, 8, 1),
                                   box_vertex.view(num_part, batch_size, 8, 3)) + center1
                box_vertex1 = box_vertex1 * parts_valid.permute(1, 0).unsqueeze(2).unsqueeze(3).repeat(1, 1, 8, 3)


            center2 = cum_pred[:, :, 0:3]
            center2 = center2.unsqueeze(2).repeat(1, 1, 8, 1)
            quat2 = cum_pred[:, :, 3:]
            box_vertex2= qrot(quat2.unsqueeze(2).repeat(1, 1, 8, 1),
                              box_vertex_input.view(num_part, batch_size, 8, 3)) + center2
            box_vertex = box_vertex2 * parts_valid.permute(1,0).unsqueeze(2).unsqueeze(3).repeat(1, 1, 8, 3)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.proj = MLPRelu(dim_in, d_model * 4, d_model, 2)
...
class TransformerRelPosEncoderLayer(nn.Module):

    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, args=None):
        super().__init__()
        self.args = args
        if not self.args.relative_box_position_encoding:
            self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        else:
            self.self_attn = AttnRelativePosition(d_model, nhead)
    ...
    def forward_post(self,
        ...
        q = k = self.with_pos_embed(src, pos)
        if self.args.relative_box_position_encoding:
            src2 = self.self_attn(src, rel_pos,   mask=src_mask,
                              key_padding_mask=src_key_padding_mask)[0]
        else:
            src2 = self.self_attn(q, k, value=src, attn_mask=src_mask,
                                  key_padding_mask=src_key_padding_mask)[0]
    ...
    def forward(self, src, rel_pos,
                src_mask: Optional[Tensor] = None,
                src_key_padding_mask: Optional[Tensor] = None,
                pos: Optional[Tensor] = None):
        if self.normalize_before:
            return self.forward_pre(src, rel_pos, src_mask, src_key_padding_mask, pos)
        return self.forward_post(src, rel_pos, src_mask, src_key_padding_mask, pos)
...
class TransformerRelPosEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        if not self.args.relative_box_position_encoding:
            ...
            self.self_attn = AttnRelativePosition(d_model, nhead, dropout=dropout)
    ...
    def forward_post(self,
        ...
        q = k = self.with_pos_embed(src, pos)
        if self.args.relative_box_position_encoding:
            src2 = self.self_attn(src, rel_pos, mask=src_key_padding_mask)
        else:
            src2 = self.self_attn(q, k, value=src, attn_mask=src_mask,
                                  key_padding_mask=src_key_padding_mask)[0]
        src = src + self.dropout1(src2)
